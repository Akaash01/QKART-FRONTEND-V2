...
const Checkout = () => {
  ...
  const validateRequest = (items, addresses) => {
    const cartTotal = getTotalCartValue(items)
    console.log({cartTotal,balance:parseInt(balance) })
    if(parseInt(balance) <cartTotal ){
      enqueueSnackbar(
        "You do not have enough balance in your wallet for this purchase",
        {
          variant: "error",
  ...
  const performCheckout = async (token, items, addresses) => {
    ...
    if(validateRequest(items,addresses)){
      ...
      let curBalance = parseInt(balance) - getTotalCartValue(items);
      localStorage.setItem('balance',curBalance);
      history.push('/thanks')
    }
  }
  catch(err){
  ...
  useEffect(() => {
    const onLoadHandler = async () => {
      ...
      const add = await getAddresses(token);
       setAddresses({...addresses,all:add});
    };
    onLoadHandler();
          ...
          <Box className="shipping-container" minHeight="100vh">
            ...
            <Box>
              {/* TODO: CRIO_TASK_MODULE_CHECKOUT - Display list of addresses and corresponding "Delete" buttons, if present, of which 1 can be selected */}
               ...
               {  
                 addresses.all.map((ad)=><div className={`address-item ${addresses.selected== ad._id ? 'selected':''}`} onClick={()=> setAddresses({...addresses,selected:ad._id})}><span>{ad.address}</span><Button onClick={()=>{let tok = localStorage.getItem('token');deleteAddress(tok,ad._id)}} startIcon={<Delete />}>DELETE</Button></div>)
               }
            </Box>
